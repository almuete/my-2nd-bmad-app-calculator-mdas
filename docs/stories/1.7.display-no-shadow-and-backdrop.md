...
6) No change to MDAS logic, layout structure, keyboard support, or ARIA/labels.  
7) Works on Desktop and Mobile Chrome baselines without layout shift or overflow.  
8) No performance regressions (paint/layout) attributable to display styling changes.
